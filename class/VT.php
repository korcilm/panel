...
class VT{
	...
	/* SELECT * FROM tabloadı WHERE ID =1 ORDER NY ID ASC LIMIT 1*/
	public function VeriGetir($tablo,$weherealanlar="", $wherearraydeger="",$orderby="ORDER BY ID ASC",$limit="")
	{
		$this->baglanti->query("SET CHARACTER SET utf8");
		$sql="SELECT * FROM ".$tablo;
		if (!empty($weherealanlar) && !empty($wherearraydeger)) {
			$sql.=" ".$weherealanlar;
